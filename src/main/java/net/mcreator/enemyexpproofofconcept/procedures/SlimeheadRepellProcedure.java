...
public class SlimeheadRepellProcedure {
	...
	private static void execute(@Nullable Event event, Entity entity, Entity sourceentity) {
		...
		if ((entity instanceof LivingEntity _entGetArmor ? _entGetArmor.getItemBySlot(EquipmentSlot.HEAD) : ItemStack.EMPTY).getItem() == EnemyexpansionModItems.SLIMEHEAD_HELMET.get()) {
			sourceentity.setDeltaMovement(new Vec3((Math.sin(Math.toRadians(entity.getYRot() + 180)) * 1.5 * 1), ((Math.sin(Math.toRadians(0 - entity.getXRot())) + 0.5) * 1), (Math.cos(Math.toRadians(entity.getYRot())) * 2 * 0.5)));
		}
	}
}
