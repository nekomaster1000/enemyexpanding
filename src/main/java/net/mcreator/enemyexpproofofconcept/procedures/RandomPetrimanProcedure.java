...
public class RandomPetrimanProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
		if (entity == null)
			return;
		if ((world.getLevelData().getGameRules().getInt(EnemyexpansionModGameRules.PETRIMANRANDOMRULE)) > 0) {
			if (entity instanceof PetrimanEntity) {
				new Object() {
					private int ticks = 0;
					...
					private void run() {
						if (Math.random() < 0.2 && entity.isAlive()) {
							if (!entity.level.isClientSide())
								entity.discard();
							if (world instanceof ServerLevel _level) {
								Entity entityToSpawn = new PetricrawlerEntity(EnemyexpansionModEntities.PETRICRAWLER.get(), _level);
						...
						} else if (Math.random() < 0.4 && entity.isAlive()) {
							if (!entity.level.isClientSide())
								entity.discard();
							if (world instanceof ServerLevel _level) {
