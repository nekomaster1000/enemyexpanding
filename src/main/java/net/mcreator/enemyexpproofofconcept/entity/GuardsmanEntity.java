...
import net.mcreator.enemyexpproofofconcept.procedures.GuardsmanAvoidanceProcedure;
import net.mcreator.enemyexpproofofconcept.init.EnemyexpansionModEntities;

import java.util.Set;

@Mod.EventBusSubscriber
public class GuardsmanEntity extends Skeleton implements IAnimatable {
	public static final EntityDataAccessor<Boolean> SHOOT = SynchedEntityData.defineId(GuardsmanEntity.class, EntityDataSerializers.BOOLEAN);
	public static final EntityDataAccessor<String> ANIMATION = SynchedEntityData.defineId(GuardsmanEntity.class, EntityDataSerializers.STRING);
	public static final EntityDataAccessor<String> TEXTURE = SynchedEntityData.defineId(GuardsmanEntity.class, EntityDataSerializers.STRING);
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.MONSTER).add(new MobSpawnSettings.SpawnerData(EnemyexpansionModEntities.GUARDSMAN.get(), 20, 1, 1));
	}

	public GuardsmanEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(EnemyexpansionModEntities.GUARDSMAN.get(), world);
	}

	public GuardsmanEntity(EntityType<GuardsmanEntity> type, Level world) {
		super(type, world);
		xpReward = 8;
		setNoAi(false);
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(TEXTURE, "guardsman");
	}
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(1, new AvoidEntityGoal<>(this, Player.class, (float) 6, 1, 1.2) {
			...
			public boolean canUse() {
				double x = GuardsmanEntity.this.getX();
				double y = GuardsmanEntity.this.getY();
				double z = GuardsmanEntity.this.getZ();
				Entity entity = GuardsmanEntity.this;
				Level world = GuardsmanEntity.this.level;
				return super.canUse() && GuardsmanAvoidanceProcedure.execute();
			}
		});
	}
	...
	public void playerTouch(Player sourceentity) {
		...
		HuntsmanGetAwayProcedure.execute(this.level, this, sourceentity);
	}
	...
	public static void init() {
		SpawnPlacements.register(EnemyexpansionModEntities.GUARDSMAN.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
				(entityType, world, reason, pos, random) -> (world.getDifficulty() != Difficulty.PEACEFUL && Monster.isDarkEnoughToSpawn(world, pos, random) && Mob.checkMobSpawnRules(entityType, world, reason, pos, random)));
		DungeonHooks.addDungeonMob(EnemyexpansionModEntities.GUARDSMAN.get(), 180);
	}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(GuardsmanEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
