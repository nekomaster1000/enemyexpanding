...
public class GladiusEntity extends Monster implements IAnimatable {
	...
	public static final EntityDataAccessor<String> TEXTURE = SynchedEntityData.defineId(GladiusEntity.class, EntityDataSerializers.STRING);
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.MONSTER).add(new MobSpawnSettings.SpawnerData(EnemyexpansionModEntities.GLADIUS.get(), 30, 1, 1));
	}
	...
	public GladiusEntity(EntityType<GladiusEntity> type, Level world) {
		super(type, world);
		xpReward = 15;
		setNoAi(false);
	}
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(TEXTURE, "gladius");
	}
	...
	public void baseTick() {
		super.baseTick();
		GladiusTransformProcedure.execute(this.level, this.getX(), this.getY(), this.getZ(), this);
	...
	public static void init() {
		SpawnPlacements.register(EnemyexpansionModEntities.GLADIUS.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
			int x = pos.getX();
			int y = pos.getY();
			int z = pos.getZ();
			return GladiusSpawningProcedure.execute(world);
		});
		DungeonHooks.addDungeonMob(EnemyexpansionModEntities.GLADIUS.get(), 180);
	}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(GladiusEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
