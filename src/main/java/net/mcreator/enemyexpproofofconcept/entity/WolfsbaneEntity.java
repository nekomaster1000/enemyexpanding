...
import net.mcreator.enemyexpproofofconcept.procedures.WolfsbaneHurtProcedure;
...
public class WolfsbaneEntity extends Monster implements IAnimatable {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.MONSTER)
					.add(new MobSpawnSettings.SpawnerData(EnemyexpansionModEntities.WOLFSBANE.get(), 10, 1, 1));
	}
	...
	public WolfsbaneEntity(EntityType<WolfsbaneEntity> type, Level world) {
		super(type, world);
		xpReward = 12;
		setNoAi(false);
	}
	...
	protected void registerGoals() {
		...
		this.targetSelector.addGoal(5, new NearestAttackableTargetGoal(this, Rabbit.class, false, false));
	...
	public boolean hurt(DamageSource source, float amount) {
		WolfsbaneHurtProcedure.execute(this.level, this.getX(), this.getY(), this.getZ(), this, source.getEntity());
	...
	public static void init() {
		SpawnPlacements.register(EnemyexpansionModEntities.WOLFSBANE.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(WolfsbaneEntity.RemovalReason.KILLED);
		}
	}
