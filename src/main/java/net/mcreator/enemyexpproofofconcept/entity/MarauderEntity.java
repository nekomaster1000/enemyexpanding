...
public class MarauderEntity extends PathfinderMob implements IAnimatable {
	...
	public MarauderEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(EnemyexpansionModEntities.MARAUDER.get(), world);
	}

	public MarauderEntity(EntityType<MarauderEntity> type, Level world) {
		super(type, world);
		xpReward = 5;
		setNoAi(false);
		...
		this.moveControl = new MoveControl(this) {
			@Override
			public void tick() {
				if (MarauderEntity.this.isInWater())
					MarauderEntity.this.setDeltaMovement(MarauderEntity.this.getDeltaMovement().add(0, 0.005, 0));
				if (this.operation == MoveControl.Operation.MOVE_TO && !MarauderEntity.this.getNavigation().isDone()) {
					double dx = this.wantedX - MarauderEntity.this.getX();
					double dy = this.wantedY - MarauderEntity.this.getY();
					double dz = this.wantedZ - MarauderEntity.this.getZ();
					float f = (float) (Mth.atan2(dz, dx) * (double) (180 / Math.PI)) - 90;
					float f1 = (float) (this.speedModifier * MarauderEntity.this.getAttribute(Attributes.MOVEMENT_SPEED).getValue());
					MarauderEntity.this.setYRot(this.rotlerp(MarauderEntity.this.getYRot(), f, 10));
					MarauderEntity.this.yBodyRot = MarauderEntity.this.getYRot();
					MarauderEntity.this.yHeadRot = MarauderEntity.this.getYRot();
					if (MarauderEntity.this.isInWater()) {
						MarauderEntity.this.setSpeed((float) MarauderEntity.this.getAttribute(Attributes.MOVEMENT_SPEED).getValue());
						float f2 = -(float) (Mth.atan2(dy, (float) Math.sqrt(dx * dx + dz * dz)) * (180 / Math.PI));
						f2 = Mth.clamp(Mth.wrapDegrees(f2), -85, 85);
						MarauderEntity.this.setXRot(this.rotlerp(MarauderEntity.this.getXRot(), f2, 5));
						float f3 = Mth.cos(MarauderEntity.this.getXRot() * (float) (Math.PI / 180.0));
						MarauderEntity.this.setZza(f3 * f1);
						MarauderEntity.this.setYya((float) (f1 * dy));
					} else {
						MarauderEntity.this.setSpeed(f1 * 0.05F);
					}
				} else {
					MarauderEntity.this.setSpeed(0);
					MarauderEntity.this.setYya(0);
					MarauderEntity.this.setZza(0);
				}
			}
		};
	}
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(TEXTURE, "marauder");
	}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(MarauderEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
