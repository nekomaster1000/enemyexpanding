...
public class RancherEntity extends Monster implements IAnimatable {
	public static final EntityDataAccessor<Boolean> SHOOT = SynchedEntityData.defineId(RancherEntity.class, EntityDataSerializers.BOOLEAN);
	public static final EntityDataAccessor<String> ANIMATION = SynchedEntityData.defineId(RancherEntity.class, EntityDataSerializers.STRING);
	public static final EntityDataAccessor<String> TEXTURE = SynchedEntityData.defineId(RancherEntity.class, EntityDataSerializers.STRING);
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.MONSTER).add(new MobSpawnSettings.SpawnerData(EnemyexpansionModEntities.RANCHER.get(), 10, 1, 1));
	}

	public RancherEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(EnemyexpansionModEntities.RANCHER.get(), world);
	}

	public RancherEntity(EntityType<RancherEntity> type, Level world) {
		super(type, world);
		xpReward = 20;
		setNoAi(false);
	}
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(TEXTURE, "goblin_rancher");
	}
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(9, new LeapAtTargetGoal(this, (float) 0.45));
		this.goalSelector.addGoal(10, new RandomLookAroundGoal(this));
		this.goalSelector.addGoal(11, new FloatGoal(this));
		this.goalSelector.addGoal(12, new RandomSwimmingGoal(this, 2, 40));
	}
	...
	public static void init() {
		SpawnPlacements.register(EnemyexpansionModEntities.RANCHER.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
				(entityType, world, reason, pos, random) -> (world.getDifficulty() != Difficulty.PEACEFUL && Monster.isDarkEnoughToSpawn(world, pos, random) && Mob.checkMobSpawnRules(entityType, world, reason, pos, random)));
		DungeonHooks.addDungeonMob(EnemyexpansionModEntities.RANCHER.get(), 180);
	}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(RancherEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
